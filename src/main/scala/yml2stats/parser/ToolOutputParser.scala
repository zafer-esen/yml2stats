...
object SVOutputParser extends ToolOutputParser {
  ...
  def apply(outputLines: Seq[String], bmName: String): Result = {
    val filteredOutput = outputLines.filter(line => line.trim.nonEmpty)
    //assert(filteredOutput.length == 1)
    ...
    var _isWallTimeout = false
    var _isKilled      = false
    val line           = filteredOutput.last
    line match {
      case _ if line.toLowerCase contains "false" =>
        if (_isSat || _isUnknown)
          throw new Exception(
            bmName + " e1: cannot determine benchmark output from lines: " + filteredOutput)
        else
          _isUnsat = true
      case _ if line.toLowerCase contains "true" =>
        if (_isUnsat || _isUnknown)
          throw new Exception(
            bmName + " e2: cannot determine benchmark output from lines: " + filteredOutput)
        else
          _isSat = true
    ...
    val result: Result =
      if (_isError || _isKilled && !considerKilledAsTimeout) {
        val errorMsg   = filteredOutput.mkString("\n")
        val errorTypes = new MHashSet[ErrorType.Value]
        for (line <- filteredOutput) {
          line match {
...
//            errorTypes += ErrorType.Other // todo: detect other errors
    ...
    //      throw new Exception(
    //      bmName + " e5: cannot determine benchmark output from lines: " + filteredOutput)
...
object TriCeraOutputParser extends ToolOutputParser {
  def apply(outputLines: Seq[String], bmName: String): Result = {
    var _isSat         = false
    var _isUnsat       = false
    var _isUnknown     = false
